...
-- Description
-- Write SQL statement to query course id, course name and instructor email in combination with course table courses and instructor table teachers and display the result columns with course id, course_name and teacher_email respectively.
...
-- id	course_name	teacher_email
-- Because the teachers in the Teacher table in Example 2 do not have a corresponding course, i.e. the teacher numbers in the two tables do not have the same item, the query does not find anything that matches the condition and returns null.
...
-- My Solution
 Select c.id as id, c.name as course_name, t.email as teacher_email
from courses as c 
Join teachers as t 
